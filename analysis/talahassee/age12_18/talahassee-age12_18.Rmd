...
## A1 - Sex-Cause
```{r sex-cause, echo=T, message=FALSE, results='show'}
g1 <- ds_sex %>% 
...
## A2 - Cause-Sex
```{r cause-sex, echo=T, results='show', message=FALSE}
g2 <- ds_sex %>% 
...
## B1 - Race-Sex-Cause
```{r race-0-sex-cause, echo=T, results='show', message=FALSE}
g1 <- ds_sex_race %>% 
  ...
  theme_bw()+
  labs(
    title = "White-Non-Hispanics: Gender differences within each mortality cause"
  )
g1 %>% print()
```

## B2 - Race-Cause-Sex
```{r race-0-cause-sex, echo=F, results='show', message=FALSE}
g1 <- ds_sex_race %>% 
  dplyr::filter(race == "white") %>% 
  ggplot(aes(x = year, y = value, color = mortality_cause) )+
  geom_point()+
  ...
  theme_bw()+
  labs(
    title = "White-Hispanics: Mortality cause within each gender"
  )
g1 %>% print()
```
...
## C1 - Sex-Cause-Race
```{r sex-0-cause-race echo=T, results='show', message=FALSE}
g1 <- ds_sex_race %>% 
  ...
  theme_bw()+
  labs(
    title = "Males: Mortality causes for each race"
  )
g1 %>% print()
```

## C2 - Sex-Race-Cause
```{r sex-0-race-cause, echo=T, results='show', message=FALSE}
g1 <- ds_sex_race %>% 
  dplyr::filter(sex == "Male") %>% 
  ...
  theme_bw()+
  labs(
    title = "Males: Race differences for each mortality cause"
  )
g1 %>% print()
```
...
## D1 - Cause-Sex-Race
```{r dev-d-0, echo=T, results='show', message=FALSE}

g1 <- ds_sex_race %>% 
  dplyr::filter(mortality_cause == "Firearms") %>% 
  ggplot(aes(x = year, y = value, color = sex) )+
  geom_point()+
  geom_line(aes(group=sex))+
  facet_grid(measure ~ race, scales = "free")+
  theme_bw()+
  labs(
    title = "Firearms: Gender differences in each race"
  )
g1 %>% print()
```

## D2 - Cause-Race-Sex
```{r dev-d-1, echo=T, results='show', message=FALSE}
...
g1 <- ds_sex_race %>% 
  ...
  theme_bw()+
  labs(
    title = "Firearms: Race differences for each gender"
  )
g1 %>% print()
```
