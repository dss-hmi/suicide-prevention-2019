...
library(readxl)
# ---- declare-globals ---------------------------------------------------------
path_input       <- "./data-unshared/raw/talahassee/"
...
ls_ds <- list(
  "1" =  readxl::read_excel(input_files[1], col_names = FALSE, skip = 3)
...
names(ds0) <- c(
  "sex","race","ethnicity","age_group","age","external","injury","mortality_cause", c(2006:2018),'total'
)
...
ds2 <- ds1 %>% 
  dplyr::filter(! external      == "Total") %>% 
  dplyr::filter(! injury      == "Total") %>% 
...
ds2 %>%          saveRDS("./data-unshared/derived/talahassee/0-full.rds")
ds2 %>% readr::write_csv("./data-unshared/derived/talahassee/0-full.csv") # for read-only inspection
